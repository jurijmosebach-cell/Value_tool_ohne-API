...
async function fetchGames(){ const res=await fetch(API_URL); const data=await res.json(); return data.response||[]; }
async function fetchTopGames(){ const res=await fetch(API_URL); const data=await res.json(); return { topValue:data.top7Value||[], topOver:data.top5Over25||[] }; }

function createBar(value,label,color){
  const wrapper=document.createElement("div"); wrapper.className="value-bar-wrapper";
  const bar=document.createElement("div"); bar.className="value-bar"; bar.style.width=`${Math.min(Math.abs(value)*50,100)}%`; bar.style.backgroundColor=color;
  const text=document.createElement("span"); text.className="bar-label"; text.textContent=`${label}: ${value}`;
  wrapper.appendChild(bar); wrapper.appendChild(text); return wrapper;
}
...
function renderGamesTable(games,container){
  ...
  games.forEach(g=>{
    ...
    row.innerHTML=`<div class="team"><img src="${g.homeLogo}" alt="${g.home}" /> ${g.home}</div>
    ...
    barsContainer.appendChild(createBar(g.btts,"BTTS",g.btts>=0?"#ff9800":"#f44336"));
...
function renderTopList(list,container,type){
  container.innerHTML="";
  list.forEach(g=>{
    const item=document.createElement("div"); item.className="top-item";
    let val=type==="value"?Math.max(g.value.home,g.value.draw,g.value.away):g.value.over25;
    let trend="";
    if(type==="value"){ if(val===g.value.home) trend="home"; else if(val===g.value.draw) trend="draw"; else trend="away"; }
