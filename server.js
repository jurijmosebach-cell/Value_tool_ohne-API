...
// League IDs laut football-data.org
const LEAGUE_IDS = {
  "Champions League": "CL",
  "Bundesliga": "BL1",
  "Eredivisie": "DED",
...
function computeOver25Prob(homeLambda, awayLambda, maxGoals=7){
  ...
  for(let i=0;i<=2;i++){
    for(let j=0;j<=2;j++){
      if(i+j<=2) pLe2+=poisson(i,homeLambda)*poisson(j,awayLambda);
    }
  }
...
async function fetchGamesFromAPI(){
  ...
  for(const [leagueName, id] of Object.entries(LEAGUE_IDS)){
    try{
      ...
      if(!res.ok){ console.error(`football-data response ${res.status} for ${leagueName}`); continue; }
      ...
      data.matches.forEach(m=>{
        ...
        // Realistische Trend-Berechnung
        let trend = "neutral";
        const maxVal = Math.max(value.home,value.draw,value.away);
        const diffHomeAway = Math.abs(prob.home-prob.away);
        if(maxVal>0.1){
          if(prob.home>prob.away && prob.home>prob.draw && diffHomeAway>0.05) trend="home";
          else if(prob.away>prob.home && prob.away>prob.draw && diffHomeAway>0.05) trend="away";
          else if(prob.draw>=Math.max(prob.home,prob.away)) trend="draw";
        }
        ...
        allGames.push({
          ...
          homeXG, awayXG, totalXG: +(homeXG+awayXG).toFixed(2),
          odds, prob, value, btts: bttsProb, trend
        });
      });
    }catch(err){ console.error("Error fetching league",leagueName,err.message); continue; }
  }
...
app.get("/api/games", async(req,res)=>{
  try{
    ...
    if(!cache.data.length || now-cache.timestamp>CACHE_DURATION){
      cache = { timestamp: now, data: await fetchGamesFromAPI() };
    }
    let filtered = cache.data.slice();
    if(req.query.date) filtered = filtered.filter(g=>g.date.startsWith(req.query.date));
    const top7Value = filtered.slice().sort((a,b)=>Math.max(b.value.home,b.value.draw,b.value.away)-Math.max(a.value.home,a.value.draw,a.value.away)).slice(0,7).map(g=>({home:g.home,away:g.away,league:g.league,value:Math.max(g.value.home,g.value.draw,g.value.away),trend:g.trend}));
    const top5Over25 = filtered.slice().sort((a,b)=>b.value.over25-a.value.over25).slice(0,5).map(g=>({home:g.home,away:g.away,league:g.league,value:g.value.over25,trend:g.trend}));
