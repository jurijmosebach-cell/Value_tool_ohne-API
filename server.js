...
if (!API_KEY) console.warn("⚠️ FOOTBALL_DATA_API_KEY ist nicht gesetzt! API-Requests werden fehlschlagen.");

let cache = { timestamp: 0, data: [] };
const CACHE_DURATION = 15 * 60 * 1000; // 15 Minuten

// Nur Ligen, die zuverlässig Daten liefern
const LEAGUES = {
  "Premier League": "PL",
  "Bundesliga": "BL1",
  "2. Bundesliga": "BL2",
...
function getFlag(team) {
  ...
  const countries = {
    "England":"gb","Germany":"de","Spain":"es","Italy":"it","France":"fr","USA":"us",
...
app.get("/api/games", async (req,res)=>{
  ...
  try{
    ...
    for(const [leagueName, code] of Object.entries(LEAGUES)){
      ...
      if(!resp.ok){
        console.warn(`⚠️ Fehler beim Abrufen von ${leagueName}: ${resp.status}`);
        continue;
      }
      const data = await resp.json();
      console.log(`ℹ️ ${leagueName}: ${data.matches.length} Spiele gefunden`);

      for(const m of data.matches){
        if(m.status === "FINISHED") continue; // nur aktuelle/laufende Spiele
