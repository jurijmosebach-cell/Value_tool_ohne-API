...
const XG_API_URL = process.env.XG_API_URL || ""; // optional: externer xG provider endpoint
const XG_API_KEY = process.env.XG_API_KEY || ""; // optional
...
/* ---------- xG provider integration ---------- */

async function fetchXGFromProvider(home, away, date){
  // Requires XG_API_URL to be set. Expects JSON { homeXG, awayXG }
  if(!XG_API_URL) throw new Error("no xg api configured");
  try{
    const url = new URL(XG_API_URL);
    url.searchParams.set("home", home);
    url.searchParams.set("away", away);
    if(date) url.searchParams.set("date", date);
    const headers = {};
    if(XG_API_KEY) headers["Authorization"] = `Bearer ${XG_API_KEY}`;
    const res = await fetch(url.toString(), { headers, timeout: 8000 });
    if(!res.ok) throw new Error(`xg provider status ${res.status}`);
    const j = await res.json();
    if(typeof j.homeXG === "number" && typeof j.awayXG === "number"){
      return { homeXG: +j.homeXG, awayXG: +j.awayXG };
    } else {
      throw new Error("xg provider returned invalid payload");
    }
  }catch(err){
    throw err;
  }
  ...
  for(const [leagueName, id] of Object.entries(LEAGUE_IDS)){
    try {
      ...
      for(const m of data.matches){
        // Try fetch real xG from provider (optional)
        let homeXG = null, awayXG = null;
        const matchDate = m.utcDate ? m.utcDate.split("T")[0] : undefined;
        if(XG_API_URL){
          try{
            const xg = await fetchXGFromProvider(m.homeTeam?.name || "", m.awayTeam?.name || "", matchDate);
            homeXG = Number(xg.homeXG);
            awayXG = Number(xg.awayXG);
          }catch(e){
            // provider failed — fallback below
            console.warn("xg provider failed for", m.homeTeam?.name, "vs", m.awayTeam?.name, e.message);
          }
        }

        // fallback generation if provider not configured or failed
